<template>
            ...
            <template #header>
                <div class="card-header">
                    ...
                    <div class="card-header-right">
                        <el-button type="primary" @click="btnClick">我的活动</el-button>
                    </div>
                </div>
            </template>
            <DataTable :column="column" :tableData="tableData" @onDel="deleteModal" @onEdit="editModal"></DataTable>
            <el-dialog v-model="addDialogVisible" title="我的活动" width="800" :before-close="handleClose">
                <DataTable :column="column" :tableData="tableData" @onDel="deleteModal" @onEdit="editModal"></DataTable>
                <template #footer>
 ...
 import { getActivityList, addActivity, editActivity, delActivity } from '@/api/activity'
 ...
 const add = async() => {
    try {
        ...
        await addActivity({activity: obj})
        ElMessageBox({
            type: 'success',
            message: "编辑成功"
        })
 ...
 const edit = async(record) => {
    try {
        ...
        await editActivity({activity: obj})
        ElMessageBox({
            type: 'success',
            message: "编辑成功"
        })
 ...
 const del = async (record) => {
    try {
        await delActivity({ id: record.id })
        ElMessageBox({
            type: 'success',
            message: "删除成功"
        })
